...
lib.createReceipt = (data) => {
  const receiptHead = `Hi ${data.name}, here is your receipt:\n`;
  let recepitContent = 'Food Desciption | Quantity  | Price | Total for Item';
  data.items.forEach((item) => {
    recepitContent += `${item.name} | ${item.quantity} | ${item.price} | ${item.price * item.quantity}\n`;
  });
  return `
${receiptHead}
${recepitContent}
This gives a total price of: ${data.total}

Thanks for choosing us! Enjoy your meal!
  `;
};
